...
\keywords{Semiring, Kleene Star, Fixed Point, Formal Verification, Rocq, Voting, Optimisation, Semimodule} %TODO mandatory; please add comma-separated list of keywords
...
in bounded semirings, formalised in the Rocq theorem prover. Our central result establishes 
...
formalised in the Rocq theorem prover and extracted to executable OCaml.
Specifically:
...
\item \textbf{Generic Rocq framework} (Section~\ref{sec:formalisation}):
We provide a Rocq library where users instantiate a semiring by
...
Section~\ref{sec:formalisation} describes the Rocq library.
...
setting in the Rocq theorem prover.
\begin{align*}
...
summation-over-$k$-terms ($a^{\langle k \rangle}$), along with their Rocq encodings as:
\end{definition}
...
\end{theorem}
We encode Theorem \ref{bnd} in Rocq as \textcolor{blue}{$astar\_exists\_gen\_q\_stable$}. The proof is by induction on $q$ 
...
We encode the set of matrices $M_{n}(R)$ in Rocq as a function type from a finite type to a finite type to \(R\), 
...
\end{center}
 ...
 \captionof{figure}{Rocq Encoding of Matrix Addition, Multiplication, and  Exponentiation}
...
\begin{definition}
   ...
   along with its Rocq encoding: $A^{\langle k \rangle} = I \oplus_{m} A \oplus_{m} A^2 \oplus_{m} \ldots \oplus_{m} A^k$.
\end{definition}
\begin{lstlisting}[language=Coq]
...
This is the closure of $A$. We have not encoded $A^{*}$ itself in Rocq, because over an arbitrary semiring there is nothing to encode: the limit need not exist. The failure has two forms. The iteration may fail to stabilise even though the limit exists; with regular languages endowed with the union and concatenation operators, $A^{*}$ is the infinite union of all the powers, and no finite stage reaches it. Over other carriers the limit does not exist at all; with natural numbers under addition and multiplication, the powers simply grow. We are only interested in cases where the iteration does reach a fixed point. Therefore, we express $A^{*}$ as
$A^{\langle k + n - 1 \rangle}$ in proofs, where $n$ is the dimension of $A$ and $k$ is an arbitrary natural number. Quantifying over $k$ is precisely the statement that the sum stops growing at stage $n - 1$, so it expresses the fixed point in finite terms. Ideally, we could have encoded $A^{*}$ as a coinductive definition and demonstrated that it reaches a fixed point using a mix of inductive and coinductive reasoning \cite{Nakata_2010}. However, this would have complicated our reasoning without significant benefit.
...
We encode Theorem \ref{thm:thm5} in Rocq as \textcolor{blue}{$matrix\_fixpoint$}. Note 
...
Similarly, the cost of a set of paths $P$ is defined as the sum of the costs of all individual paths in $P$ and  expressed as: $ \textcolor{blue}{sum\_all\_rvalues} (P) = 
  \bigoplus_{\pi \in P} \text{ } \textcolor{blue}{measure\_of\_path} (\pi)$. The Rocq encodings of these definitions are shown below, where the cost of an empty path (\(\epsilon\)) is represented by \(\overline{1}\), and the cost of an empty set (\(\{\}\)) is denoted by \(\overline{0}\).
...
This formalisation is illustrated on the left side and its corresponding Rocq encoding is presented on 
the right side of Figure \ref{fig:definitionofpaths}. 
The Rocq encoding takes two 
extra argument the adjacency matrix $A$ and its size $n$, 
enumerated in a list $finN$, i.e., $finN = \{0, 1, \dots, n-1\}$. 
This is because we encode a path $(c, c_1, c_2 \dots )$ in Rocq as a list 
...
\captionof{figure}{Definition of Path Set (left) and its Rocq Encoding (right)}
...
We formally establish in Rocq that $A^{k}(i, j) = \bigoplus_{p \in P^{k}(i, j)} \, \textcolor{blue}{measure\_of\_path}(p)$, expressed 
...
We encode Theorem \ref{thm:thm8} in Rocq as below and in the inductive case, 
...
but we show here only the Rocq encoding of the Wikipedia example. This choice is intentional because the Wikipedia page has been scrutinised by many experts, including Markus Schulze himself, making it a reliable example. We define the candidates
...
shown here-- all the axioms of semiring, including boundedness. In order to get an executable machine code, we extract the \textcolor{blue}{$wikipedia$} function into an OCaml code using the Rocq's extraction facility and call it from 
...
\end{lstlisting}
\end{minipage}
\captionof{figure}{Rocq Encoding of the Schulze Election from Wikipedia}
\label{fig:coq}
\begin{center}
...
of the Rocq theorem prover to use our library,  specifically discharging the 
...
established by Schulze \cite{schulze2011new} using the Rocq theorem prover.
...
can easily be established in Rocq. Another future 
...
\section{Rocq Formalisation}
\label{sec:formalisation}

Our Rocq development consists of approximately 5,000 lines of code organised
into three layers:
...
We use Rocq's extraction mechanism to produce executable OCaml code from
each verified instantiation. The extraction process translates Rocq
...
We can combine two semirings to construct a new, more complex semiring by leveraging the abstract nature of our formalisation. For example, the Rocq code shown below defines a shortest-widest path semiring, where we aim to find paths that are both the shortest in terms of distance and the widest in terms of capacity. The semiring is built by combining two algebraic structures: one for the shortest path problem (using the min-plus algebra) and one for the widest path problem (using the max-min algebra).  The graph is constructed with three nodes $A$, $B$, and $C$, and 
